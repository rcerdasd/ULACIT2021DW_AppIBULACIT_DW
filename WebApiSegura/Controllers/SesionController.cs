    ...
    public class SesionController : ApiController
    {
        ...
        public IHttpActionResult GetId(int id)
        {
            ...
            try
            {
                ...
                {
                    SqlCommand sqlCommand = new SqlCommand(@"SELECT Codigo, CodigoUsuario, FechaInicio, FechaExpiracion, Estado FROM Sesion WHERE Codigo = @Codigo", sqlConnection);
        ...
        public IHttpActionResult GetAll()
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"SELECT Codigo, CodigoUsuario, FechaInicio, FechaExpiracion, Estado FROM Sesion", sqlConnection);
                    sqlConnection.Open();
        ...
        public IHttpActionResult Ingresar(Sesion sesion)
        {
            ...
            try
            {
                using (SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["INTERNET_BANKING"].ConnectionString))
                {
                    SqlCommand sqlCommand = new SqlCommand(@"INSERT INTO Sesion (CodigoUsuario, FechaInicio, FechaExpiracion, Estado) VALUES (@CodigoUsuario, @FechaInicio, @FechaExpiracion, @Estado)", sqlConnection);
